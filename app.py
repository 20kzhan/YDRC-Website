...
logger = logging.getLogger("YDRC Backend")
...
def add_app_metadata(id, metadata: dict):
    url = f'https://{env.get("AUTH0_DOMAIN")}/api/v2/users/{id}'
...
def home_page():
    ...
    if user:
        ...
        if 'app_metadata' not in user:
            print("no app_metadata found, defaulting to student account type")
            user['app_metadata'] = {'account_type': 'student'}
            add_app_metadata(sub, {'account_type': 'student'})
...
def teacher():
    ...
    sub = session['user']['userinfo']['sub']
    classes = get_classes(sub) # Replace this and the getting stuff in admin() with a single function
